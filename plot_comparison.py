...
cleo.utils.style_plots_for_paper()
...
class PlotData:
    def __init__(self, stim_t, spike_counts_in_radius, spike_vals, stim_vals):
        ...
        self.ground_truth_spikes = spike_counts_in_radius
        self.recorded_spikes = spike_vals
        self.opto_stim = stim_vals


obj_with_opto_on = PlotData(
    stim_t=data_opto_on["stim_t"],
    spike_counts_in_radius=data_opto_on["spike_counts_in_radius"],
    spike_vals=data_opto_on["spike_vals"],
    stim_vals=data_opto_on["stim_vals"],
)
obj_with_opto_off = PlotData(
    stim_t=data_opto_off["stim_t"],
    spike_counts_in_radius=data_opto_off["spike_counts_in_radius"],
    spike_vals=data_opto_off["spike_vals"],
    stim_vals=data_opto_off["stim_vals"],
)
...
line_detect = ax.plot(
    obj_with_opto_on.time_axis,
    obj_with_opto_on.recorded_spikes,
    "k-",
    label="with opto",
)
line_detect = ax.plot(
    obj_with_opto_off.time_axis,
    obj_with_opto_off.recorded_spikes,
    "-",
    color=".5",
    label="without opto",
)
...
for i in range(len(obj_with_opto_on.opto_stim) - 1):
    if obj_with_opto_on.opto_stim[i] > 0:
        opto_line = ax.plot(
            [obj_with_opto_on.time_axis[i], obj_with_opto_on.time_axis[i + 1]],
            [ylim[1], ylim[1]],
            color=light_473nm,
            ms=4,
        )
...
# line_radius=ax_one.plot(stim_t[0:(len(stim_t)-1)],spike_counts_in_radius,'k-')
line_detect = ax_2.plot(
    obj_with_opto_on.time_axis, obj_with_opto_on.ground_truth_spikes, "k-"
)
line_detect = ax_2.plot(
    obj_with_opto_off.time_axis, obj_with_opto_off.ground_truth_spikes, "k-", color=".5"
)
