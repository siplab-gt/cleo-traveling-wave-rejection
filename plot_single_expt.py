...
def plot_all(excitespikes, numofexcneur, results_dir):
    # TODO: refactor to pass in ax for flexible figure creation
    t_samps = [0, 4, 8, 12]
    fig, axs = plt.subplots(1, len(t_samps))
    for ax, t_samp in zip(axs, t_samps):
        smooth_std = 0.2
        visualize_spiking_per_ms_smoothed(
            excitespikes, numofexcneur, t_samp, smooth_std, ax
        )
